...
bool PhysicalBoard::isOccupied(int row, int col) const { 
  ...
  return bb_getBit(occupied, square);
}
...
Bitboard parseBoardPosition(const char* pieces) {
  ...
  while (*p) {
    ...
    // Skip piece character (P, N, B, R, Q, K or lowercase)
    if ((*p >= 'A' && *p <= 'Z') || (*p >= 'a' && *p <= 'z')) p++;
    
    // Parse square (e.g., "a1", "e4")
