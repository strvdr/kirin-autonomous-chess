...
Bitboard parsePosition(const char* pieces) {
  ...
  while (*p) {
    ...
    if (*p >= 'a' && *p <= 'h' && *(p+1) >= '1' && *(p+1) <= '8') {
      ...
      setBit(occ, coord.toSquareIndex());
      p += 2;
    }
  }
  return occ;
...
Bitboard startingPosition() {
  ...
  for (int col = 0; col < 8; col++) {
    setBit(occ, 7 * 8 + col);  // Rank 1
    setBit(occ, 6 * 8 + col);  // Rank 2
    setBit(occ, 1 * 8 + col);  // Rank 7
    setBit(occ, 0 * 8 + col);  // Rank 8
  }
  return occ;
}
...
void testStartingPosition() {
  ...
  // King move (1 square)
  {
    ...
    clearBit(occ, BoardCoord::fromFEN("e2").toSquareIndex());
