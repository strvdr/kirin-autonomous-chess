...
\title{%
    ...
    {\large Version 0.2.0 (Development Build)}
}
...
This document provides comprehensive documentation for the \textbf{Kirin Chess Engine}. Kirin is a chess engine written in C++ and an integral part of the \textbf{First Move Robotics} autonomous chess board project.
...
Dependencies & POSIX libraries (unistd.h, sys/time.h) \\
\bottomrule
\end{tabular}
...
git clone git@github.com:strvdr/first-move-robotics.git
cd engine
mkdir -p build
cd build && cmake .. && make
./kirin
