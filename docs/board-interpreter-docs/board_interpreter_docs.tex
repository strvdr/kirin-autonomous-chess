...
\title{%
    ...
    {\large Version 0.3.1 (Development Build)}
}
\author{Strydr Silverberg\\strydr\_silverberg@mines.edu\\\\Colorado School of Mines, Class of 2026}
\date{\today}
...
\begin{enumerate}
    ...
    \item \textbf{Plan restorations}: iterate over \texttt{relocations} in reverse order (LIFO). For each relocation, call \texttt{findClearPath} from the parking spot back to the piece's original square. If no clear path exists, the path is left empty and the gantry controller falls back to a direct move.
